...
Then read the row for `permittruck.xyz`. Or pull from
...
## 2026-05-23 — Food truck operators actively search for their city's permit…
- **Status:** active
- **Hypothesis:** Food truck operators actively search for their city's permit requirements because the rules are confusing, scattered across agencies, and the cost of getting them wrong is a fine or a shutdown. By publishing genuinely useful per-city and per-permit pages that rank for those searches, PermitTruck reaches operators at the exact moment they feel the pain — and converts a fraction into free signups, then paid. Because the search audience is the buyer (unlike a booking tool, where SEO reaches the customer's customer), the content compounds into a self-serve acquisition channel that needs no paid spend. The bet is that this loop, plus reminder emails that demonstrate ongoing value, drives retention past the seasonal-churn risk inherent to the vertical.
- **KPI:** any GSC traffic — clicks, impressions, indexed-page count
- **Baseline:** 0 clicks / 0 impressions (just deployed)
- **Action:** project scaffolded via `portfolio new bootstrap`; first deploy pending. After deploy: verify in GSC as `sc-domain:permittruck.xyz` and submit the sitemap.
- **Result:** TBD — review 2026-06-20
- **Learning:** TBD
